...
export const GoogleDrive = {
     ...
     */
    async uploadFile(fileName, mimeType, base64Data, parentFolderId = null) {
        try {
            const token = await this.getProviderToken();
            if (!token) {
            ...
            // Use provided folderId or fallback to AardappelLapse folder
            let folderId = parentFolderId;
            if (!folderId) {
                folderId = await this.getOrCreateFolder(token);
            }

            if (!folderId) {
                console.error("[GoogleDrive] Could not find or create destination folder.");
            ...
            if (!response.ok) {
                ...
                console.error("[GoogleDrive] Drive API Error:", errText);
                throw new Error(`DRIVE_API_ERROR: ${response.status}`);
            }

            const result = await response.json();
            console.log("[GoogleDrive] Successfully uploaded file:", result.id);
            return result.id;
        } catch (error) {
            console.error("[GoogleDrive] Upload File Exception:", error.message);
            throw error;
        }
    }
};
